...
contract DAOConfig is IDAOConfig, Ownable
	...
	// The amount of ZERO tokens provided as a bootstrapping reward when a new token is whitelisted.
	// Note that new tokens will only be able to be whitelisted (even if the vote is favorable) once the DAO ZERO token balance is at least this amount.
	// The DAO receives 25 million ZERO tokens (linearly over 10 years - about 48k per week)
	// Range: 50k ether to 500k ether with an adjustment of 50k ether
	uint256 public bootstrappingRewards = 200000 ether;

	// The percent of ZERO token rewards that are burned.
	// Range: 5% to 15% with an adjustment of 1%
    uint256 public percentRewardsBurned = 10;

	// The minimum amount of veZERO required for ballot quorum (to take action on the ballot).
	// baseBallotQuorum is decided as a percent of the total amount of ZERO tokens staked.
	// NOTE: there is a hard minimum of 0.50% of ZERO.totalSupply that takes effect if the amount of staked ZERO tokens is low (at launch for instance).
	// Parameter adjustment quorum: = 1 * baseBallotQuorum
	// Token whitelisting quorum: = 2 * baseBallotQuorum
	// Sending ZERO tokens from the DAO quorum: = 3 * baseBallotQuorum
	...
	uint256 public baseBallotQuorumPercentTimes1000 = 7 * 1000; // Default 7% of the total amount of ZERO tokens staked with a 1000x multiplier
	...
	// The percent of staked ZERO tokens that a user has to have to make a proposal
	...
	// The percentage of ZERO token rewards that are sent to the DAO's reserve
	// Range: 5% to 15% with an adjustment of 1%
