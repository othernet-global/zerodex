...
contract DAO is IDAO, Parameters, ReentrancyGuard
    ...
    event ZeroSent(address indexed to, uint256 amount);
	...
	uint256 constant SEND_ZERO_COOLDOWN = 1 weeks;
	...
	// The timestamp that ZERO tokens were last sent from the DAO
	uint256 public nextValidTimestampSendZERO;
	...
	function _executeApproval( Ballot memory ballot ) internal
		...
		else if ( ballot.ballotType == BallotType.SEND_ZERO )
			{
			// Make sure the contract has the ZERO token balance before trying to send it.
			// This should not happen but is here just in case - to prevent approved proposals from reverting on finalization.
			if ( exchangeConfig.zero().balanceOf(address(this)) >= ballot.number1 )
				{
				IERC20(exchangeConfig.zero()).safeTransfer( ballot.address1, ballot.number1 );

				nextValidTimestampSendZERO = block.timestamp + SEND_ZERO_COOLDOWN;

				emit ZeroSent(ballot.address1, ballot.number1);
				}
			}
	...
	function _finalizeTokenWhitelisting( uint256 ballotID ) internal
		...
		if ( proposals.ballotIsApproved(ballotID ) )
			...
			// Make sure that the DAO contract holds the required amount of ZERO tokens for bootstrappingRewards.
			// Twice the bootstrapping rewards are needed (for both the token/WETH and token/USDC pools)
			uint256 zeroBalance = exchangeConfig.zero().balanceOf( address(this) );
			require( zeroBalance >= bootstrappingRewards * 2, "Whitelisting is not currently possible due to insufficient bootstrapping rewards" );
			...
			exchangeConfig.zero().approve( address(liquidityRewardsEmitter), bootstrappingRewards * 2 );
	...
	function finalizeBallot( uint256 ballotID ) external nonReentrant
		...
		if ( ballot.ballotType == BallotType.PARAMETER )
			...
			{
			// ZERO tokens can only be sent from the DAO once a week to combat DAO reserve draining attacks.
			if ( ballot.ballotType == BallotType.SEND_ZERO )
			if ( block.timestamp < nextValidTimestampSendZERO )
				return; // return and allow the Send ZERO to be executed later
