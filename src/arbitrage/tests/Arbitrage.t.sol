...
contract TestArbitrage is Deployment
	...
	constructor()
		...
		zero.transfer(alice, 1000000 ether);
		vm.stopPrank();
		}
	...
	function _setupTokenForTesting( IERC20 token ) public
		...
		zero.transfer(alice, 100000 ether);
		...
		liquidity.depositLiquidityAndIncreaseShare( zero, weth, 100 ether, 100 ether, 0, 0, 0, block.timestamp, false );
		liquidity.depositLiquidityAndIncreaseShare( weth, usdc, 100 ether, 100 * 10**6, 0, 0, 0, block.timestamp, false );
		liquidity.depositLiquidityAndIncreaseShare( zero, usdc, 100 ether, 100 * 10**6, 0, 0, 0, block.timestamp, false );
	...
	function _setupTokenForTestingNoLiquidity( IERC20 token ) public
		...
		// Whitelist the tokens with SALT and WETH
        vm.startPrank(address(dao));
        poolsConfig.whitelistPool(  token, zero);
        poolsConfig.whitelistPool(  token, weth);
        vm.stopPrank();
		...
		liquidity.depositLiquidityAndIncreaseShare( token, zero, 100 ether, 100 ether, 0, 0, 0, block.timestamp, false );
		liquidity.depositLiquidityAndIncreaseShare( token, weth, 100 ether, 100 ether, 0, 0, 0, block.timestamp, false );
		liquidity.depositLiquidityAndIncreaseShare( zero, weth, 100 ether, 100 ether, 0, 0, 0, block.timestamp, false );
	...
	function testArbitrage1() public
		{
		assertEq( pools.depositedUserBalance( address(dao), weth ), 0, "starting deposited zero balance should be zero" );
...
//		console.log( "profit: ", pools.depositedUserBalance( address(dao), zero ) );
		assertTrue( pools.depositedUserBalance( address(dao), zero ) > 283* 10**15, "arbitrage profit too low" );
		}
...
//		console.log( "profit: ", pools.depositedUserBalance( address(dao), zero ) );
	...
	function testArbitrageFailed() public
		{
		assertEq( pools.depositedUserBalance( address(dao), zero ), 0, "starting deposited zero balance should be zero" );
		...
		uint256 startingSALT = zero.balanceOf(alice);
		uint256 amountOut = pools.depositSwapWithdraw( weth, zero, 10 ether, 0, block.timestamp );

		// Check the swap itself (prices not accurate)
		assertEq( amountOut, 9090909090909090909 );
		assertEq( zero.balanceOf(alice) - startingSALT, 9090909090909090909 );

		(uint256 reservesA0,) = pools.getPoolReserves(zero, weth);

		assertTrue( reservesA0 == (100 ether - amountOut), "Arbitrage should not happen" );
		assertEq( pools.depositedUserBalance( address(dao), zero ), 0, "There should be no arbitrage profit" );
		}
	...
	function testSeriesOfTrades() public
		...
		for( uint256 i = 0; i < 20; i++ )
			{
			uint256 startingDepositSALT = pools.depositedUserBalance( address(dao), zero );

			pools.depositSwapWithdraw( zero, weth, 1 ether, 0, block.timestamp );
			rollToNextBlock();

			uint256 profit = pools.depositedUserBalance( address(dao), zero ) - startingDepositSALT;
...
//			console.log( i, profit );
	...
	function _setupTokenForTestingMin( IERC20 token ) public
		...
		liquidity.depositLiquidityAndIncreaseShare( token, zero, 100000, 100000, 0, 0, 0, block.timestamp, false );
		liquidity.depositLiquidityAndIncreaseShare( zero, usdc, 100000, 100000, 0, 0, 0, block.timestamp, false );
		liquidity.depositLiquidityAndIncreaseShare( weth, usdc, 100000, 100000, 0, 0, 0, block.timestamp, false );
		liquidity.depositLiquidityAndIncreaseShare( zero, weth, 100000, 100000, 0, 0, 0, block.timestamp, false );
	...
	function testArbitrageReservesMin() public
		...
		uint256 startingSALT = zero.balanceOf(alice);
		uint256 amountOut = pools.depositSwapWithdraw( weth, zero, 10000, 0, block.timestamp );

		// Check the swap itself
		assertEq( amountOut, 9090 );
        assertEq( zero.balanceOf(alice) - startingSALT, 9090 );

        // Check that the arbitrage swaps happened as expected
        (uint256 reservesA0, uint256 reservesA1) = pools.getPoolReserves(zero, weth);
        (uint256 reservesB0, uint256 reservesB1) = pools.getPoolReserves(weth, usdc);
        (uint256 reservesC0, uint256 reservesC1) = pools.getPoolReserves(usdc, zero);
...
//		console.log( "profit: ", pools.depositedUserBalance( address(dao), zero ) );
		assertEq( pools.depositedUserBalance( address(dao), zero ), 282, "arbitrage profit incorrect" );
		}
	}
