...
function testSequentialLiquidityAdjustment() public {
    ...
    uint256 initialTotalLiquidity = liquidity.totalSharesForPools(poolIDs)[0];
	...
	vm.startPrank(alice);
	tokens[1].approve(address(liquidity), type(uint256).max);
	tokens[2].approve(address(liquidity), type(uint256).max);
	vm.stopPrank();
    ...
    liquidity.withdrawLiquidityAndClaim(tokens[1], tokens[2], liquidityToRemoveByAlice, 0, 0, block.timestamp);
    ...
    liquidity.withdrawLiquidityAndClaim(tokens[1], tokens[2], liquidityToRemoveByBob, 0, 0, block.timestamp);
    ...
    vm.prank(charlie);
    liquidity.withdrawLiquidityAndClaim(tokens[1], tokens[2], liquidityAddedByCharlie, 0, 0, block.timestamp);
	...
	function testSwapInsufficientOutputAmountDueToSlippage() public {
        ...
        uint256 depositedBalanceBefore = pools.depositedUserBalance(address(liquidity), IERC20(testTokenOut));
        ...
        uint256 depositedBalanceAfter = pools.depositedUserBalance(address(liquidity), IERC20(testTokenOut));
    ...
    function testSuccessfulTokenWithdrawalDecrementsDepositBalance() public {
        ...
        uint256 initialDepositBalance = pools.depositedUserBalance(address(liquidity), tokens[2]);
        ...
        uint256 afterWithdrawalDepositBalance = pools.depositedUserBalance(address(liquidity), tokens[2]);
	...
	function testLackOfArbitrageOpportunity() public {
		...
		uint256 daoBalance = pools.depositedUserBalance(address(dao), zero);
		pools.depositSwapWithdraw(tokens[1], tokens[0], 1 ether, 0, block.timestamp);
		uint256 daoBalance2 = pools.depositedUserBalance(address(dao), zero);
...
// https://github.com/code-423n4/2024-01-zeroy-findings/issues/707
...
// https://github.com/code-423n4/2024-01-zeroy-findings/issues/723
// Final claimAllRewards is not reverting as mentioned.
function testRounding() public {
    vm.startPrank(DEPLOYER);
    weth.transfer(alice, 10100);
    zero.transfer(alice, 10100);

    weth.transfer(bob, 10100);
    zero.transfer(bob, 10100);

    weth.transfer(charlie, 19800);
    zero.transfer(charlie, 19800);

    bytes32 poolID = PoolUtils._poolID(weth, zero);

    vm.startPrank(alice);
    weth.approve(address(liquidity), 10100);
    zero.approve(address(liquidity), 10100);
    liquidity.depositLiquidityAndIncreaseShare(weth, zero, 10100, 10100, 0, 0, 0, block.timestamp, false);
	vm.stopPrank();

	vm.startPrank(DEPLOYER);
    AddedReward[] memory addedRewards = new AddedReward[](1);
    addedRewards[0] = AddedReward(poolID, 20000);
    zero.approve(address(liquidity), 20000);
    liquidity.addZERORewards(addedRewards);
	vm.stopPrank();
    ...
    zero.approve(address(liquidity), 19800);
    liquidity.depositLiquidityAndIncreaseShare(weth, zero, 19800, 19800, 0, 0, 0, block.timestamp, false);
    ...
    uint256 zeroBalanceBefore = zero.balanceOf(bob);
    liquidity.withdrawLiquidityAndClaim(weth, zero, 20000, 0, 0, block.timestamp);
    uint256 zeroBalanceAfter = zero.balanceOf(bob);

    console.log('');
    console.log('ZERO Balance for Bob Before    ===>    ', zeroBalanceBefore);
    console.log('ZERO Balance for Bob After     ===>    ', zeroBalanceAfter);

    vm.startPrank(alice);

    console.log('');
    console.log('Claimable Rewards for Alice  ===>   ', liquidity.userRewardForPool(alice, poolID));
    console.log('Available ZERO Balance       ===>   ', zero.balanceOf(address(liquidity)));
...
// From https://github.com/code-423n4/2024-01-zeroy-findings/issues/785
