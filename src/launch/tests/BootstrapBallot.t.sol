...
contract TestBootstrapBallot is Deployment
	...
	function test_finalizeBallot() public {
		...
		assertEq( zero.balanceOf(address(initialDistribution)), 100000000 ether);
	...
	function test_finalizeBallotAlreadyFinalized() public {
        ...
        // Verify that the InitialDistribution.distributionApproved() was called.
		assertEq( zero.balanceOf(address(initialDistribution)), 0 ether);
    }
...
//		bootstrapBallot.finalizeBallot();
	...
	function testAirdrop2() public
		...
		bool eq1 = (bootstrapBallot.claimableTimestamp1() + 45 days) == bootstrapBallot.claimableTimestamp2();
		bool eq2 = (bootstrapBallot.claimableTimestamp1() + 60 minutes) == bootstrapBallot.claimableTimestamp2();

		assertTrue( eq1 || eq2, "Airdrop 2 not claimable at the correct time" );
		...
		uint256 zeroAlice0 = zero.balanceOf(alice);
		uint256 zeroBob0 = zero.balanceOf(bob);
		uint256 zeroCharlie0 = zero.balanceOf(charlie);
		...
		assertEq( zero.balanceOf(alice) - zeroAlice0, 1000 ether, "Incorrect amount claimed for alice" );
		assertEq( zero.balanceOf(bob) - zeroBob0, 2000 ether, "Incorrect amount claimed for bob" );
		assertEq( zero.balanceOf(charlie) - zeroCharlie0, 3000 ether, "Incorrect amount claimed for charlie" );
		}
    ...
    function testAuthorizeAirdrop2CalledOnce() public {
        ...
        vm.prank(alice);
        bootstrapBallot.authorizeAirdrop2(zeroAmount, validSignatureAlice);
