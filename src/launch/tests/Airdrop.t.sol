...
contract TestAirdrop is Deployment
	...
	function testAllowClaiming() external {
        ...
        // Get initial zero balance of Airdrop contract
        uint256 initialZeroBalance1 = zero.balanceOf(address(airdrop1));
        uint256 initialZeroBalance2 = zero.balanceOf(address(airdrop2));
        assertEq( initialZeroBalance1, 4000000 ether);
        assertEq( initialZeroBalance2, 3000000 ether);
	...
	function testClaimAirdrop() external {
		...
		uint256 aliceBalance = zero.balanceOf( alice );
		...
		uint256 aliceClaimed = zero.balanceOf( alice ) - aliceBalance;
    ...
    function testSuccessiveClaimsUpdateClaimedPerUserCorrectly() external {
            ...
            uint256 zeroAmount = 1000 ether;

            // Authorize user wallet with specific zeroAmount before claiming is allowed
            vm.prank(address(bootstrapBallot));
            airdrop1.authorizeWallet(user, zeroAmount);
            ...
            uint256 expectedClaimedAmountFirst = zeroAmount / 2;
            ...
            assertEq(airdrop1.claimedByUser(user), zeroAmount, "Second claim amount incorrect");
    }
    ...
    // A unit test to verify that the constructor sets the exchangeConfig and zero variables as expected
	function testConstructorSetsExchangeConfigAndZeroAsExpected() external {
        IExchangeConfig expectedExchangeConfig = exchangeConfig; // Assuming `exchangeConfig` is accessible in the test environment.
        IZero expectedZero = zero; // Assuming `zero` is accessible in the test environment.
        ...
        // Retrieves the actual `exchangeConfig` and `zero` from the deployed `Airdrop` instance.
        IExchangeConfig actualExchangeConfig = airdropTestInstance.exchangeConfig();
        IZero actualZero = airdropTestInstance.zero();

        // Asserts that the actual `exchangeConfig` and `zero` are as expected.
        assertEq(address(actualExchangeConfig), address(expectedExchangeConfig), "Constructor did not set exchangeConfig as expected.");
        assertEq(address(actualZero), address(expectedZero), "Constructor did not set zero as expected.");
    }
